...
ActionParser.initialise()
  .then(() => Promise.all(intrinsicPaths.map(path => DirectoryUtility.fileNamesInDirectory(path, /\.as$/i))))
  .then(_ => {
    fileList = (<string[][]><any>_).reduce((acc, val) => acc.concat(val), []);
    return Promise.all(fileList.map(path => LoadQueue.enqueue(path)));
  })
  .then(_ => {
    ...
    for (let i = 0, l = fileList.length; i < l; i++) {
      pickleSrc += ActionParser.parseFile(fileList[i], fileContents[i]).toIntrinsicPickle().replace(jsonPropMatcher, '$1:');
      pickleSrc += i === l-1 ? '\n];' : ',\n  ';
    }
